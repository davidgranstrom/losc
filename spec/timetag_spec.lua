...
describe('Timetag', function()
  describe('constructors', function()
    it('can create a raw timetag', function()
      ...
      assert.are.equal(0, tt:seconds())
      assert.are.equal(1, tt:fractions())
      tt = Timetag.new_raw(0)
      assert.are.equal(1, tt:fractions())
      tt = Timetag.new_raw(1, 2)
      assert.are.equal(1, tt:seconds())
      assert.are.equal(2, tt:fractions())
    end)
  ...
  describe('methods', function()
    ...
    it('can access seconds and fractions', function()
      local now = os.time()
      local tt = Timetag.new(now)
      assert.is_true(type(tt:seconds()) == 'number')
      assert.is_true(type(tt:fractions()) == 'number')
    end)
  end)
end)
